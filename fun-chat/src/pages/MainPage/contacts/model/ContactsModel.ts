...
import { allActiveUsers, allInactiveUsers, messageHistory } from '../../../../core/socket/actions/user-actions';
...
export default class ContactsModel {
  ...
  private drawUsers() {
    ...
    currentUserDialogs.forEach((dialog) => {
      const unreadMessagesCount = dialog.messages.filter((message) => !message.status.isReaded).length;
  ...
  private subscribeToEvents() {
    ...
    this.observer.subscribe(ObserverEvents.messageSend, (message) => this.handleMessages(message));
  }
}
